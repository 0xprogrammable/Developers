...
- Publish additive Direct Native Hook Graph Profile Revision 3 discovery for the live Custom Launch API V3 general lane: exact source/compiler/graph binding, blocking-versus-warning static indicators, mandatory exact Router simulation before authorization, CLI contract `3.1.0`, retained Revision 2 compatibility, and explicit non-audit, non-honeypot, non-liquidity, non-tradeability, and non-fee-behavior guarantees. The immutable CLI release locator remains intentionally unpublished until its artifact exists.
