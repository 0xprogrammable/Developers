...
- Publish the immutable checksum-bound CLI `3.3.7` locator and bind it to tarball SHA-256 `905fff9464586cd1bf1873a0b8edb292603ffc4e07a4f7eef6e0fa2b6cb32fe8`, byte length `271430`, and Custom Launch V3 OpenAPI `3.3.7` SHA-256 `06add50990f9966dd77de3e7700ce8087acca7b8a5c74f70c97f48ebc7c2b3b7`; retain older CLI entries below as release history.
- Describe additive profile `3.4.0` only as pending release readiness: it cannot be selected until the runner and observation ABI are deployed and read back, server-executed assertions remain server-owned, and the CLI auto-injects and validates the canonical fee vault. Existing `3.3.0` reads and exact retries keep their original semantics.
- Correct the finalized Custom metadata source boundary to `finalized-v3-project-metadata-ledger`, retaining metadata-bearing `3.2.0` and `3.3.0` rows while excluding pending and metadata-absent historical resources.
