...
- If the Router Custom identity commitment changes, an `after` poll replays the bounded Router identity set; deduplicate normalized records by chain ID plus `launchId` before committing the new resume cursor.
...
- Upsert normalized launch records by chain ID plus `launchId`; retain chain ID plus Router address plus `launchId` as the exact Router provenance identity.
