...
When the Robinhood V4 lane is promoted, each record retains its independent source-verification state under `extensions["programmable/backend-finalized-v4"].sourceVerification`. Router provenance and Ethereum finality do not imply an exact source match. Only a durable Sourcify V2 exact result can publish `exactMatchProvider: "sourcify-v2"`; Blockscout evidence alone cannot.
