...
Robinhood Chain Mainnet (`eip155:4663`) publishes a live canonical Router and
finalized launch evidence for independent terminal discovery. Its direct-chain
integration uses `platformId: "programmable"`, `category: "custom"` and the
public label `Programmable Custom`. The hosted read model and self-serve V4
API/CLI remain `planned`; this release does not promote public writes.
...
| Custom Launch API V4 for Robinhood Chain | Existing Programmable bearer credential when promoted | Chain-bound `pack`, `validate`, `submit`, `status` contract with separate wallet signing | [Robinhood manifest](https://developers.programmable.family/api/v2/manifests/4663); its self-serve V4 descriptor remains `planned`, with a [byte-pinned V4 OpenAPI mirror](openapi/custom-launch-v4.json) of the expected release contract |
...
| [Per-chain manifests](https://developers.programmable.family/api/v2/manifests/1) | Bind deployment and feed discovery to the selected `chainId`; chain 4663 publishes live direct-chain provenance independently of its planned hosted read model |
...
`ProgrammableLaunchStampRouterV1` is the trust root for Router-stamped Programmable Classic and Programmable Custom launches only on chains whose exact manifest marks that Router `live`. Ethereum and Robinhood publish separate live Router roots. For Robinhood, require `directChainIntegration.status: "live"` and the exact chain-4663 Router evidence; the hosted read model may remain unavailable while an external terminal verifies and indexes stamps directly.
...
For Robinhood, fetch `directChainIntegration.evidenceUrl` from the chain-4663
manifest and verify its existing finalized launch through the published Router.
Keep that evidence separate from the Ethereum vectors below. The
[Robinhood terminal integration](https://developers.programmable.family/robinhood-terminal-indexer)
provides the direct-chain verification and future-launch indexing sequence.

Run the [Robinhood release verifier](examples/verify-robinhood-release.mjs)
from a clone of this repository with Node.js 20 or later; no package install is
required:
...
The helper discovers the manifest and derives its existing finalized token
from the canary receipt; it requires no copied address or new launch. Use an
RPC that serves canonical finalized and historical hash-bound reads. The
verifier never substitutes `latest` for finality.
A successful result includes `state: "stamped"`, `category: "custom"` and
`publicLabel: "Programmable Custom"`; an incomplete check fails closed.

Use the Finalized PCAN vector below to smoke-test an Ethereum implementation:
...
When the Robinhood hosted V4 read model is promoted, each record retains its independent source-verification state under `extensions["programmable/backend-finalized-v4"].sourceVerification`. Router provenance and finality do not imply an exact source match. The V4 `exact_match` state requires the independent protected-source, hosted-build, finalized-transaction and bytecode binding; Sourcify V2 and Blockscout observations alone cannot grant it.
